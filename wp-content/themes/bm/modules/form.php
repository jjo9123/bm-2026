...
<?php if( get_sub_field('recent_choice') == 'recent'): ?>
	<section class="blog recent <?php echo $bg; ?>" style="background: #404040 url('/wp-content/uploads/2019/01/recent_bg.jpg') 50%/cover no-repeat; color: #FFFFFF;">
		<div class="container-fluid <?php echo $cat_slug; ?>">
			<div class="container">
				<div class="row">
					...
					<?php if( $query->have_posts() ) : while( $query->have_posts() ) : $query->the_post(); $categories = get_the_category($mypost->ID);
						if ( ! empty( $categories ) ) {
					   $category = esc_html( $categories[0]->slug );
					   $category1 = esc_html( $categories[1]->slug );
					   $name = esc_html( $categories[0]->name );
					 	}; ?>
						<div class="col-sm-6 col-md-4 text-center item i<?php echo $category; ?><?php if ( ! empty( $category1 ) ) { echo ' ' . $category1; }; ?>">
							...
							<div class="excerpt">
                              <?php if($categories[0]->slug == 'events'): ?>
									<p><?php $excerpt = get_field('intro_title'); echo wp_trim_words( $excerpt, 30, '...' ); ?></p>
								<?php elseif($categories[0]->slug == 'training'): ?>
									<p><?php $excerpt = get_field('intro_title'); echo wp_trim_words( $excerpt, 30, '...' ); ?></p>

                                <?php elseif(get_field('new_blog_layout') == 'yes'): ?>
                                  <p><?php $new_excerpt = get_field('blog_intro'); echo wp_trim_words( $new_excerpt, 30, '...' ); ?></p>
					...
					<?php foreach ($myposts as $mypost): $categories = get_the_category($mypost->ID);
					   ...
					   $name = esc_html( $categories[0]->name );
						...
						<div class="col-sm-6 col-md-4 text-center item i<?php echo $category; ?><?php if ( ! empty( $category1 ) ) { echo ' ' . $category1; }; ?>">
							...
							<div class="excerpt">
								<?php if($categories[0]->slug == 'events'): ?>
									<p><?php $excerpt = $mypost->intro_title; echo wp_trim_words( $excerpt, 30, '...' ); ?></p>
								<?php elseif($categories[0]->slug == 'training'): ?>
									<p><?php $excerpt = $mypost->intro_title; echo wp_trim_words( $excerpt, 30, '...' ); ?></p>
                                <?php elseif( $mypost->new_blog_layout == 'yes'): ?>
                                    <p><?php $excerpt = $mypost->blog_intro; echo wp_trim_words( $excerpt, 30, '...' ); ?></p>
								<?php else: ?>
