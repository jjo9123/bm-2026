...
$build_items = function ($rows) use ($to_id) {
  ...
  foreach ($rows as $row) {
    ...
    if (!$exp_id || get_post_status($exp_id) !== 'publish') {
      continue;
    }
...
$org_items = in_array('organisations', $tabs_to_show, true) ? $build_items($org_rows) : [];
$ind_items = in_array('individuals', $tabs_to_show, true) ? $build_items($ind_rows) : [];
                      ...
                      >
                        <div class="card-body">
                          ...
                          <?php if (!empty($services)) : ?>
                            <ul>
                              <?php foreach ($services as $svc) :
                                $svc_id = $to_id($svc);
                                if (!$svc_id || get_post_status($svc_id) !== 'publish') {
                                  continue;
                                }
                              ?>
                                <li>
                          ...
                          <?php else : ?>
                            <p>
                              <a href="<?php echo esc_url(get_permalink($exp_id)); ?>" class="expertise-link">
                                Learn more about <?php echo esc_html(get_the_title($exp_id)); ?>
                              </a>
                            </p>
                          <?php endif; ?>
                        </div>
